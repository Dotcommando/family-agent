...
## Стэкинг сообщений и событий

Если пока агент занят, из одного чата приходит несколько сообщений, они не должны превращаться в независимые reasoning-циклы.

Ожидаемое поведение:
- сообщения из одного чата собираются в батч в пределах окна batching/coalescing
- более поздние сообщения могут отменять ранние инструкции
- reasoning должен запускаться по объединённому контексту, а не по каждому сообщению отдельно
- пока есть пользовательские события в очереди, low-priority thought loop не должен начинать новую итерацию

Для этого в `.env` предусмотрены настройки:
- `AGENT_MESSAGE_BATCH_WINDOW_SECONDS`
- `AGENT_CHAT_COALESCE_MAX_ITEMS`
- `AGENT_EVENT_QUEUE_STRATEGY`
- `AGENT_THOUGHT_LOOP_SKIP_WHEN_QUEUE_NOT_EMPTY`
